...
class FieldValidator:
    ...
    BACKEND_VALIDATED_FIELDS = {
        "dob", "date_of_birth",
        "dateOfBirth",
        "mobile", "mobile_number", "nok_phone_number", "phone_number", "phoneNumber",
        "email",
    ...
    def requires_backend(cls, field_name: str) -> bool:
        # Backend is authoritative for every field; keep the allow-list for
        # backward compatibility and documentation, but enforce for all fields.
        return True
    ...
    def _run(cls, field: str, value: Any, ctx: Dict) -> Optional[str]:
        ...
        if field in ("nationality", "citizenship"):
            if ctx.get("__product_id") == "personal_accident":
                return cls._enum(value, {"Ugandan", "Non-Ugandan"}, "Nationality")
        ...
        # ---- dates ----
        if field in ("dob", "date_of_birth", "dateOfBirth"):
        ...
        if field in ("year_of_manufacture", "yearOfManufacture"):
            return cls._int_range(value, 1980, date.today().year, "Year of manufacture")
    ...
    def _phone(value: Any) -> Optional[str]:
        ...
        if not re.fullmatch(r"(?:07\d{8}|2567\d{8})", raw):
            return "Use 07XXXXXXXX or 2567XXXXXXXX."
        return None
    ...
    @staticmethod
    def _number_range(value: Any, min_val: float, max_val: float, label: str) -> Optional[str]:
        raw = _strip(value)
        if not raw:
            return f"{label} is required."
        try:
            ...
            return f"{label} must be a valid number."
        return None if min_val <= n <= max_val else f"{label} must be between {int(min_val)} and {int(max_val)}."
...
class StepValidator:
    ...
    def validate(
        ...
        context = {**(context or {}), "__product_id": product_id}
...
class FieldDecorator:
    ...
    def _enhance(cls, field: Dict[str, Any], errors: Dict[str, str]) -> Dict[str, Any]:
        ...
        if name in ("national_id_number", "nok_id_number"):
            ...
            hint["pattern"] = r"^(07\d{8}|2567\d{8})$"
            hint["patternMessage"] = "Use 07XXXXXXXX or 2567XXXXXXXX (digits only)"
            f.setdefault("placeholder", "07XXXXXXXX")
            f.setdefault("inputMode", "numeric")
        ...
        elif name in ("year_of_manufacture", "yearOfManufacture"):
            f["min"] = 1980
            f["max"] = date.today().year
            f.setdefault("inputMode", "numeric")
            hint["minValueMessage"] = "Year must not be before 1980"
            hint["maxValueMessage"] = "Year cannot be beyond the current year"

        elif name in ("vehicle_value", "vehicle_value_ugx", "vehicleValue", "vehicleValueUgx"):
            f["min"] = 10_000_000
            f["max"] = 100_000_000
            f.setdefault("inputMode", "numeric")
            hint["minValueMessage"] = "Vehicle value must be at least UGX 10,000,000"
            hint["maxValueMessage"] = "Vehicle value must not exceed UGX 100,000,000"
