...
class PaymentService:
    ...
    def enrich_transaction_metadata(self, reference: str, metadata: Dict[str, Any]) -> None:
        """Merge metadata into an existing transaction without changing its status."""
        if not metadata:
            return
        transaction = self.db.get_payment_transaction_by_reference(reference)
        if not transaction:
            return
        current_status = str(getattr(transaction, "status", PaymentStatus.PENDING.value) or PaymentStatus.PENDING.value)
        self.db.update_payment_transaction_status(
            reference,
            current_status,
            provider_reference=str(getattr(transaction, "provider_reference", "") or ""),
            metadata=metadata,
        )

    def _send_consolidated_policy_confirmation(
        self,
        *,
        reference: str,
        transaction: Any,
        ...
        tx_metadata = self._extract_transaction_metadata(transaction)
        if tx_metadata.get("policy_confirmation_email_sent"):
            return {
                "sent": False,
                "reason": "already_sent",
            }
        ...
        recipient = str(
            tx_metadata.get("customer_email")
            or tx_metadata.get("email")
            or tx_metadata.get("payer_email")
            or ""
        ).strip()
        policy_data = dict(tx_metadata.get("policy") or {})
        if not recipient:
            return {
                "sent": False,
        ...
        if "customer_name" not in policy_data:
            policy_data["customer_name"] = str(tx_metadata.get("customer_name") or "Customer")
        if "premium_amount" not in policy_data:
            policy_data["premium_amount"] = float(getattr(transaction, "amount", 0.0) or 0.0)

        payment_payload = {
            "reference": reference,
            "status": target_status,
            "amount": float(getattr(transaction, "amount", 0.0) or 0.0),
            "currency": str(getattr(transaction, "currency", "UGX") or "UGX"),
            "provider_reference": str(getattr(transaction, "provider_reference", "") or ""),
        }
        email_result = email_service.send_policy_confirmation_email(
            to_email=recipient,
            policy_payload=policy_data,
            payment_payload=payment_payload,
        )

        if email_result.get("sent"):
            self.enrich_transaction_metadata(
                reference,
                {
                    "policy_confirmation_email_sent": True,
                    "policy_confirmation_email_result": email_result,
                },
            )
        return email_result
    ...
    def apply_webhook_callback(self, payload: Dict[str, Any], signature: str) -> Dict[str, Any]:
        ...
        if current_status == PaymentStatus.PENDING.value and target_status in {
            ...
            email_result = None
            if target_status == PaymentStatus.SUCCESS.value:
                email_result = self._send_consolidated_policy_confirmation(
                    reference=reference,
                    transaction=updated or transaction,
            ...
            return {
                ...
                "policy_confirmation_email": email_result,
            }
