...
async def get_impact_metrics(
    ...
    The computation lives in ``src/metrics_kpis.py`` so the daily Zoho CRM
    metrics push uses the exact same math as this endpoint (single source of
    truth — they can never drift apart).
    """
    from src.metrics_kpis import compute_impact_metrics

    return compute_impact_metrics(db, days)
...
async def admin_metrics_push_zoho(
    ...
    try:
        ...
        logger.error("Zoho metrics push failed: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Zoho metrics push failed: {e}")
