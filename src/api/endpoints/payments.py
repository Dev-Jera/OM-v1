...
def _extract_customer_email(metadata: Dict[str, Any], underwriting_data: Dict[str, Any]) -> str:
    return str(
        metadata.get("customer_email")
        or metadata.get("email")
        or underwriting_data.get("email")
        or underwriting_data.get("email_address")
        or ""
    ).strip()


def _extract_customer_name(metadata: Dict[str, Any], underwriting_data: Dict[str, Any]) -> str:
    explicit_name = str(metadata.get("customer_name") or "").strip()
    if explicit_name:
        return explicit_name

    first_name = str(
        underwriting_data.get("first_name")
        or underwriting_data.get("firstName")
        or underwriting_data.get("name")
        or ""
    ).strip()
    surname = str(
        underwriting_data.get("surname")
        or underwriting_data.get("last_name")
        or underwriting_data.get("lastName")
        or ""
    ).strip()
...
async def run_underwrite_quote_policy_payment(
    *,
    ...
    customer_email = _extract_customer_email(metadata, underwriting_data)
    customer_name = _extract_customer_name(metadata, underwriting_data)
    ...
    if preview_result.get("declined"):
        return {
            "declined": True,
            "decision_status": preview_result.get("decision_status"),
            "underwriting": preview_result.get("underwriting"),
        }

    underwriting = normalize_underwriting_response(preview_result.get("underwriting") or {})
    quotation = normalize_quotation_response(
        preview_result.get("quotation") or {},
    ...
    if payment_enabled and payment_before_policy:
        ...
        payment_service.enrich_transaction_metadata(
            quotation.quote_id,
            {
                "policy": policy,
                "customer_email": customer_email,
