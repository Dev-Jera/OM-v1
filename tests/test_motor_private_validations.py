...
async def test_vehicle_and_premium_field_rules(motor_flow):
    ...
    # Year of manufacture cannot be beyond current year
    bad_future_year = payload.copy()
    bad_future_year["yearOfManufacture"] = date.today().year + 1
    with pytest.raises(FormValidationError) as exc1b:
        await motor_flow.complete_flow(bad_future_year, user_id="user123")
    assert "yearOfManufacture" in exc1b.value.field_errors
    ...
    # Vehicle value must be within 10M-100M UGX
    bad_value = payload.copy()
    bad_value["vehicleValueUgx"] = "9000000"
    ...
    bad_value_high = payload.copy()
    bad_value_high["vehicleValueUgx"] = "120000000"
    with pytest.raises(FormValidationError) as exc5:
        await motor_flow.complete_flow(bad_value_high, user_id="user123")
    assert "vehicleValueUgx" in exc5.value.field_errors
...
async def test_vehicle_details_form_exposes_backend_validation_metadata(motor_flow):
    ...
    assert by_name["year_of_manufacture"]["backendValidation"] is True
...
def test_motor_premium_helper_maps_guided_fields_into_engine_payload(motor_flow):
    payload = motor_flow._build_motor_premium_payload(
        {
            "about_you": {
                "first_name": "John",
                "surname": "Doe",
            ...
            "vehicle_details": {
                ...
                "car_usage_region": "Within East Africa",
            },
            "excess_parameters": ["excess_2"],
            "additional_benefits": ["political_violence"],
        }
    )

    assert payload["vehicle_value_ugx"] == "45000000"
    assert payload["car_usage_region"] == "Within East Africa"
    assert payload["selected_benefits"] == ["political_violence"]
    assert payload["excess_choice"] == ["excess_2"]


def test_motor_private_premium_uses_guided_state_values(motor_flow):
    low = motor_flow._calculate_motor_private_premium(
        {
            "vehicle_details": {
            ...
            "excess_parameters": ["excess_1"],
            "additional_benefits": [],
        }
    )
    high = motor_flow._calculate_motor_private_premium(
        {
            "vehicle_details": {
    ...
    assert high["base_premium"] > low["base_premium"]
