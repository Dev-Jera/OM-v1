...
async def test_personal_details_render_stays_on_current_step_and_has_gender_options(flow):
    ...
    nationality_field = next(field for field in result["response"]["fields"] if field["name"] == "nationality")

    assert all(field.get("backendValidation") is True for field in fields)
    ...
    assert nationality_field["type"] == "select"
    assert nationality_field["options"] == [
...
async def test_upload_national_id_accepts_ref_value_and_returns_confirmation(flow):
    data = {
        "quick_quote": {
            ...
            "mobile": "0772123456",
            "dob": "1990-01-01",
            "policy_start_date": _tomorrow_iso(),
