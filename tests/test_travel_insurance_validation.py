...
@pytest.mark.asyncio
async def test_travel_party_selection_with_extra_keys_still_renders_two_dobs(flow):
    payload = {
...
@pytest.mark.asyncio
async def test_travel_party_missing_second_dob_returns_inline_error_form(flow):
    payload = {
        "travel_party": "myself_and_someone_else",
...
@pytest.mark.asyncio
async def test_travel_party_group_missing_total_returns_inline_error_form(flow):
    payload = {
        "travel_party": "group",
        "total_travellers": "",
        "departure_country": "Uganda",
...
@pytest.mark.asyncio
async def test_travel_party_equal_return_date_is_allowed(flow):
    payload = {
        "travel_party": "myself_only",
        "traveller_1_date_of_birth": "1990-01-01",
        "departure_country": "Uganda",
...
async def test_travel_party_selection_only_renders_group_counts(flow):
    ...
    assert "num_travellers_18_69" not in fields
...
async def test_about_you_valid_saves_data_and_returns_travel_party_form(flow):
    ...
    # Valid submission auto-advances to product selection
...
@pytest.mark.asyncio
async def test_data_consent_missing_terms_is_nullable(flow):
    payload = {
    ...
    assert data["data_consent"]["terms_and_conditions_agreed"] is False
...
async def test_traveller_details_valid_single_advances_to_emergency_contact(flow):
    ...
    # After one traveller, advances to emergency contact (step 5)
...
@pytest.mark.asyncio
async def test_traveller_details_after_consent_shows_only_missing_primary_fields(flow):
    data = {
        "travel_party_and_trip": {
            "total_travellers": 1,
            "traveller_1_date_of_birth": "1990-04-11",
        },
        "about_you": {
            "first_name": "Pre",
            "surname": "Filled",
    ...
    result = await flow._step_traveller_details({}, data, "user-1")
    field_names = {f["name"] for f in result["response"]["fields"]}

    assert "first_name" not in field_names
    assert "surname" not in field_names
...
@pytest.mark.asyncio
async def test_traveller_details_nationality_radio_has_value_keys(flow):
    result = await flow._step_traveller_details({}, _trip_data(1), "user-1")
    fields = {f["name"]: f for f in result["response"]["fields"]}
    options = fields["nationality_type"]["options"]

    assert all("value" in opt for opt in options)
    assert {opt["value"] for opt in options} == {"ugandan", "non_ugandan"}


@pytest.mark.asyncio
async def test_travel_party_primary_applicant_must_be_at_least_18(flow):
    payload = {
        "travel_party": "myself_only",
        "traveller_1_date_of_birth": "2012-01-01",
        "departure_country": "Uganda",
        "destination_country": "Portugal",
        "departure_date": "2026-03-08",
        "return_date": "2026-03-10",
    ...
    assert "traveller_1_date_of_birth" in exc.value.field_errors
...
def test_bank_details_optional_step_removed(flow):
    assert not hasattr(flow, "_step_bank_details_optional")
